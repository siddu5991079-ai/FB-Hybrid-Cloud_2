...
// 👈 NAYA UPDATE: Title agar bohat lamba hoga toh auto-cut (truncate) ho jayega
let rawTitle = (process.env.VIDEO_TITLE || "Live_Match")
    .replace(/[^\w\s-]/g, '') 
    .trim()
    .replace(/\s+/g, '_');

if (rawTitle.length > 40) {
    rawTitle = rawTitle.substring(0, 40); // 40 words se lamba naam nahi jayega
}
const VIDEO_TITLE = rawTitle;
...
async function main() {
    ...
    while (true) {
        ...
        if (success) {
            ...
            try {
                // Ab permission aur name length ka error nahi aayega!
